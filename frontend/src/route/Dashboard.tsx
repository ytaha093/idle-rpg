...
function Dashboard() {
  const { equipmentDialog, itemDialog, view } = useSelector(
    (state: RootState) => ({
      equipmentDialog: state.uiData.equitmentPopup,
      itemDialog: state.uiData.itemPopup,
      view: state.uiData.currentView,
    }),
    shallowEqual,
  );
    ...
    <div className="bg-black bg-[url('assets/bg.jpg')] bg-repeat-y bg-center bg-size-[1350px]">
      ...
      <div className="max-w-[1350px] min-h-[calc(100dvh-28px)] px-17.5 m-auto max-[1350px]:px-1 max-[1350px]:max-w-[1218px]">
        <div
          ...
          className=" shadow-[10px_15px_20px_-5px_rgb(0_0_0/0.3),3px_8px_10px_0px_rgb(0_0_0/0.3)]"
        >
          <div
            data-section="banner"
            className="w-full bg-black border-0 border-stone-800 border-x-0 border-t-0"
          >
            <img src={logo} alt="" className="w-8/10 mx-auto" />
          </div>

          <div
            data-section="main buttons"
            className="w-full flex gap-[-10px] text-center"
          >
            {[
              "Home",
              "Inventory",
              ...
              return (
                <div
                  key={name}
                  className={`bg-grey1 border border-stone-800 flex-1 hover:bg-grey2 hover:cursor-pointer ${
                    view.includes(name)
                      ? "text-rsgreen hover:text-rsgreenlight"
                      : "hover:text-white"
                  }`}
                  onClick={() => dispatch(setCurrentView(name))}
                >
                  {name}
                </div>
              );
          ...
          <div className="flex bg-grey1">
            <div data-section="left sidebar" className="w-2/10">
              ...
              <CollapsibleBox boxName="Skills">
                ...
                <div className="font-pixel text-sm text-neutral-400">
                  Total Level:{" "}
