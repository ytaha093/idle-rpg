...
function EquipmentDialog({ item }: { item: EquipmentSlot | null }) {
    ...
    const armorComponent = useSelector((state: RootState) => state.invData.ArmorComponent)
    const weaponComponent = useSelector((state: RootState) => state.invData.WeaponComponent)
    ...
    const EQUIPMENT_SLOTS = ["MainWeapon", "OffWeapon", "Helm", "Armor", "Gauntlets", "Legs", "Boots"]
    ...
    function getToolInfo(tool: ToolSlot) {
        switch (tool) {
            case "Pickaxe": return { skill: "Mining", material: "Metal", ammount: metal }
            case "Hatchet": return { skill: "Woodcutting", material: "Wood", ammount: wood }
            case "Hammer": return { skill: "Quarrying", material: "Stone", ammount: stone }
        }
    }
    ...
    if (isEquipmentSlot(item)) {
        ...
        header = (<>
            <div className="font-pixel flex items-center gap-1 text-sm px-4 py-3  ">
                ...
                <div className="text-rsgreen text-left text-[0.9rem] px-0.5">{equipment.quality}</div>
        ...
        content = (<>
            <div className="flex gap-2.5 justify-center">
                ...
                <div className="flex flex-col">
                    <span>12</span>
                    <span>3%</span>
                    <span className="text-rsgreen font-medium">13</span>
                </div>
            </div>
            ...
            <div className="w-full flex gap-2 mt-1">
                <div className="text-xs p-1.5 border border-grey5 bg-grey1 rounded flex flex-col flex-1 justify-center  items-center">
                    <div className="flex items-center gap-1 text-xs">
                        <div className="text-rsgreen text-left text-[0.7rem]">{equipment.quality}</div>
                        <div className="text-xs">{name}</div>
                        <div>[<span className="text-rsgreen font-semibold ">{equipment.level + 1}</span>]</div>
                    </div>
                    <span>{isWeapon ? "Power:" : "Armor:"} 15</span>

                    <div className="mt-1.5"><ItemTag item={"Gold"} />
                        <span className={`pl-0.5 ${canUpgradeLevel ? "text-rsgreenlight" : "text-red font-semibold"}`}>{goldCost}</span>
                    </div>
                ...
                <div className="text-xs p-1.5 border border-grey5 bg-grey1 rounded flex flex-col flex-1 justify-center  items-center">
                    <div className="flex items-center gap-1 text-xs">
                        <div className="text-rsgreen text-left text-[0.7rem]">{equipment.quality + 1}</div>
                        <div className="text-xs">
                            {isWeapon ? getWeaponName(equipment.quality + 1) : getArmorName(equipment.quality + 1, item as ArmorSlot)}
                        </div>
                        <div>[<span className="text-rsgreen font-semibold ">{equipment.level}</span>]</div>
                    </div>
                    <span>Modifier: 6%</span>

                    <div className="mt-1.5"><ItemTag item={isWeapon ? "WeaponComponent" : "ArmorComponent"} />
                        <span className={`pl-0.5 ${canUpgradeType ? "text-rsgreenlight" : "text-red font-semibold"}`}>{componentCost}</span>
                    </div>
            ...
            <div className="w-full flex gap-2 mt-1">
                <div className="text-xs py-2 px-1 border border-grey5 bg-grey1 rounded flex flex-col flex-1 justify-between items-center">
                    ...
                    <div className="flex flex-col justify-center items-center">
                        ...
                        <div className="mt-1.5"><ItemTag item={material as ItemId} />
                            <span className={`pl-0.5 ${canUpgradeLevel ? "text-rsgreenlight" : "text-red font-semibold"}`}>{materialCost}</span>
                        </div>
                ...
                <div className="text-xs py-2 px-1 border border-grey5 bg-grey1 rounded flex flex-col flex-1 justify-between items-center">
                    ...
                    <div className="flex flex-col justify-center items-center">
                        ...
                        <div className="mt-1.5"><ItemTag item={"ToolComponent"} />
                            <span className={`pl-0.5 ${canUpgradeType ? "text-rsgreenlight" : "text-red font-semibold"}`}>{componentCost}</span>
                        </div>
                    ...
                    <div className="relative z-10 animate-scaleInFast w-3/10">
                        <div className=" text-left bg-grey2 text-sm shadow-lg">
                            ...
                            <header className=" bg-grey3 border-[#373737] border-b flex justify-center text-lg font-bold font-pixel">
                                {header}
                                <button className=" absolute top-px right-0 mb-3 px-3 text-[#AAAAAA] hover:text-[#DDDDDD] hover:cursor-pointer font-pixelold text-xl" onClick={fadeOutAndClose}>x</button>
                            </header>
