...
function EquipmentTag({ item }: { item: ArmorSlot | WeaponSlot }) {
  ...
  const name = item.includes("Weapon")
  ...
  return (
    <EquipmentTooltip item={item}>
      <div
        onClick={() => dispatch(setEquitmentPopup(item))}
        className="flex justify-between px-0.5 text-xs hover:cursor-pointer hover:bg-grey2 transition-all duration-100 h-4"
      >
        <div className="flex items-center">
          <img src={getIcon(item)} alt="" className="h-9/10" />
          <div className="text-rsgreen text-left text-[0.7rem] px-0.5">
        ...
        <div>
          ...
          <span className="text-rsgreen font-semibold ">
            {equipment.level.toLocaleString()}
          </span>
          ]
        </div>
      </div>
