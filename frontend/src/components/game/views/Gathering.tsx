...
function Gathering() {
  ...
  const activeSkill = useSelector(
    (state: RootState) => state.playerData.activeSkill,
  );
  ...
  function changeResultsView(view: string) {
      ...
      <div className="flex flex-col gap-3 m-auto">
        {skills.map((skill) => {
          ...
          return (
            <div
              key={skill.name}
              className={`w-full border rounded-xs overflow-hidden bg-grey1 ${
              ...
              >
                <div className="flex items-center gap-3">
                  <img
                    src={skill.icon}
                    alt={`${skill.name} icon`}
                    className="inline-block max-h-10 max-w-10"
                  />
                ...
                <div className="flex items-center gap-4">
                  <div
                    className="text-xs hover:cursor-pointer text-[#7ae] hover:text-[#58c]"
                    onClick={() => {
                      startGather(skill.name);
                      changeResultsView(skill.name); //changes view for results display
                    }}
                  >
                    Start {skill.name}
                  </div>
                ...
                <div className="px-3 pt-2 pb-3 border-t border-stone-700 bg-grey2 flex justify-between">
                  ...
                  <div className="w-4/9">
                    ...
                    {skill.loot.map((loot, index) => (
                      <div
                        key={index}
                        className="pt-px text-[0.8rem] flex items-center font-pixel font-normal text-greywhitedim2"
                      >
