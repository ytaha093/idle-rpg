  ...
  function getIconForSkill(name: SkillName) {
    switch (name) {
      case "Battling":
        return battleIcon;
      case "Dungeoneering":
        return dungioneeringIcon;
      case "Mining":
        return miningIcon;
      case "Woodcutting":
        return woodcuttingIcon;
      case "Quarrying":
        return quarryingIcon;
      case "Runecrafting":
        return runecraftingIcon;
      case "Jewelcrafting":
        return jewelcraftingIcon;
      case "Herblore":
        return herbloreIcon;
      default:
        return battleIcon;
    ...
    <div className=" text-left">
      <div className=" font-semibold text-sm flex items-center gap-2">
        <img
          ...
          alt={`${skill} icon`}
          className="inline-block max-h-5 max-w-6"
        />
        <span>{skill}</span>
      </div>
      <div className="flex gap-2.5">
        ...
        <div className="flex flex-col">
          ...
          <span>{xpRemaining.toLocaleString()}</span>
        </div>
      </div>

      <div
        data-section="tooltip"
        className="min-w-36 h-5.5 bg-linear-0 to-stone-900 from-grey4 border border-stone-800 relative my-0.5 rounded-xs"
      >
        <div
          id="meter"
          className="w-full h-full bg-g bg-linear-0 to-[#392f23] from-[#5c4b38]"
