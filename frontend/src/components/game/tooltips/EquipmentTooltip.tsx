...
function EquipmentTooltip({
  ...
  const name =
    item == "MainWeapon" || item == "OffWeapon"
      ? getWeaponName(equipment.quality)
      : getArmorName(equipment.quality, item);
  ...
  const power = 123423424;
  const modifier = 12123123;
  ...
  const content = (
    <div className=" text-left max-w-90 font-inter relative">
      ...
      <div>
        ...
        (
        <span className="text-rsgreen font-semibold">
          {modifier.toLocaleString()}
        </span>
        )
      </div>
      <div>{modifier.toLocaleString()}</div>
      <img
        src={getIcon(item)}
        alt=""
