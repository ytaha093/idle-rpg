...
function ToolTooltip({ children, item }: { children: ReactNode, item: "Hammer" | "Pickaxe" | "Hatchet" }) {
    ...
    function getMaterial(name: string) {
        switch (name) {
            case "Hammer": return "Stone"
            case "Pickaxe": return "Metal"
            case "Hatchet": return "Wood"
        }
    }
    function getSkill(name: string) {
        switch (name) {
            case "Hammer": return "Quarrying"
            case "Pickaxe": return "Mining"
            case "Hatchet": return "Woodcutting"
        }
    }
    ...
    const content = (
        <div className=" text-left max-w-90 font-inter relative">
            <div className="font-pixel flex gap-1 text-sm hover:cursor-pointer hover:bg-grey2 transition-all duration-100">
                <div className="text-rsgreen text-left text-[0.8rem]/5">{equipment.quality}</div>
                <div className="font-semibold">{getToolName(equipment.quality, item)}</div>
